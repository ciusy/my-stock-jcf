...
public class SfZyInvoiceContentDaoImpl extends DaoSupport<SfzyinvoiceContent>
		implements SfZyInvoiceContentDao {

	public void testTable() throws Exception {
		getSession().createSQLQuery(
				"insert into  SfStatistics values('1','1',0,0,0)")
				.executeUpdate();
	...
	public List<List<String>> getStatistics(String startTime, String endTime,
		...
		sb.append("select '收费员:'+t2.SName as c,t.projectName as c1,sum(t.projectMoney) as c3,"
				+ "sum(t.projectReduceMoney) as c4,sum(t.projectMoney-t.projectReduceMoney) as c5"
				+ " from SFZYInvoiceContent t,SFZYInvoiceHead t1,OSStuff t2 ");

		if (!"0".equals(costType)) {
			sb.append(" ,GHCostType t3");
		}
		// sb.append(" where t.invoiceNO=t1.invoiceNO and t1.cashierID=t2.SID and t1.iscancel='false' ");
		sb.append(" where t.invoiceNO=t1.invoiceNO and t1.cashierID=t2.SID ");
	...
	public void getInpatientStatisticsByOffice(String startTime,
		...
		sb.append("select '开出科室:'+t2.UName as c,t.projectName,sum(t.projectMoney) as c3,"
				+ "sum(t.projectReduceMoney) as c4,sum(t.projectMoney-t.projectReduceMoney) as c5"
				+ " from SFZYInvoiceContent t,SFZYInvoiceHead t1,OSUForm t2,YFMExbankA t5,YFExbankAItem t6 ");

		if (!"0".equals(costType)) {
			sb.append(" ,GHCostType t3");
		}
		sb.append(" where t2.uid=t5.au and t5.AFormID=t6.AFormID "
				+ " and t6.ItemID=billNO and  t.invoiceNO=t1.invoiceNO  ");
	...
	private void setOfficeDispose(String startTime, String endTime, String costType) {
		StringBuilder sb = new StringBuilder();
		List<Object> paramList = new ArrayList<Object>();
		...
		sb.append("select '开出科室:'+t2.UName as c,t.projectName,sum(t.projectMoney) as c3,"
				+ "sum(t.projectReduceMoney) as c4,sum(t.projectMoney-t.projectReduceMoney) as c5"
				+ " from SFZYInvoiceContent t,SFZYInvoiceHead t1,OSUForm t2,CZDispose t5 ");

		if (!"0".equals(costType)) {
			sb.append(" ,GHCostType t3");
		}
		sb.append(" where t2.uid=t5.AU "
				+ " and t5.ItemID=billNO and  t.invoiceNO=t1.invoiceNO  ");
	...
	public void getInpatientStatisticsByDoctor(String startTime,
			String endTime, String costType) {
		...
		sb.append("select '开出医生:'+t4.SName as c,t.projectName,sum(t.projectMoney) as c3,"
				+ "sum(t.projectReduceMoney) as c4,sum(t.projectMoney-t.projectReduceMoney) as c5"
				+ " from SFZYInvoiceContent t,SFZYInvoiceHead t1,OSStuff t4,YFMExbankA t5,YFExbankAItem t6 ");
	...
	private void setDoctorDispose(String startTime, String endTime,
		...
		sb.append("select '开出医生:'+t4.SName as c,t.projectName,sum(t.projectMoney) as c3,"
				+ "sum(t.projectReduceMoney) as c4,sum(t.projectMoney-t.projectReduceMoney) as c5"
				+ " from SFZYInvoiceContent t,SFZYInvoiceHead t1,OSStuff t4,CZDispose t5 ");
